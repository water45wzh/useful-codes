...
def get_arguments():
    ...
    parser.add_argument('--align', type=str, default=DEFAULT_MODE,
                        help='dtw, cut')
...
def generate_cut_feature(ref_file, test_file, new_file, vecsize, isBinary):

    ref_data = None
    test_data = None
    new_data = []
    if isBinary:
        ref_data = load_bin_param(ref_file, vecsize)
        test_data = load_bin_param(test_file, vecsize) 
    else:
        ref_data = load_param(ref_file)
        test_data = load_param(test_file)

    if len(ref_data) > len(test_data):
        # copy the last few frames:
        index = 0
        while index < len(ref_data):
            while index < len(test_data):
                new_data += [test_data[index]]
                index += 1

            new_data += [test_data[len(test_data)-1]]
            index += 1
    elif len(ref_data) < len(test_data):
        # cut off the last few frames:
        index = 0
        while index < len(ref_data):
            new_data += [test_data[index]]
            index += 1
...
def evaluate_two_waves(record_wave, generated_wave, offline_tool_dir, data_path, args):
    ...
    lsp_aligned_file = os.path.join(generaterd_data_path, 'feature', basename + '.aln.lsp')
    f0_aligned_file = os.path.join(generaterd_data_path, 'feature', basename + '.aln.f0')
    ...
    if args.align == 'dtw':
        ...
        # just cut off the last few frames
        generate_cut_feature(lsp_file,lsp_generated_file, lsp_aligned_file, LSP_order + 1, True)
        generate_cut_feature(f0_file, f0_generated_file, f0_aligned_file, 1, False)
 
    # convert f0 to lf0 as binary file
    lf0_file = os.path.join(record_wave_data_path, 'feature', basename + '.lf0')
    lf0_aligned_file = os.path.join(generaterd_data_path, 'feature', basename + '.aln.lf0')
...
def main():
    ...
    #data_path = os.path.join(file_path, 'output')
